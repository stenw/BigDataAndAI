...
Plot performance vs configurations:
...
Select best configuration (by ROC AUC):
  ...
  ```{r}
best_boost <- select_best(boost_tuned, "roc_auc")
best_boost
```

**Discussion:**  
  Do you see a trade-off between ROC AUC and Brier score?
...
nested_results <- cv_outer %>%
  mutate(
    ...
    best = map(tuned, select_best, "roc_auc"),
    ...
    auc = map_dbl(assessed, roc_auc, truth = diabetes, .pred_1),
    brier = map_dbl(assessed, brier_class, truth = diabetes, .pred_1)
  )
```
...
**Discussion prompts:**
  ...
  - Which model do you expect to be more stable across settings?
...
- **Nested CV** is required for honest performance estimation
